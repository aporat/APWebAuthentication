...
public final class FiveHundredspxWebAuthViewController: AuthViewController {
    // MARK: - Data

    fileprivate var auth: FiveHundredspxWebAuthentication
    fileprivate var loggedIn = false

    // MARK: - UIViewController

    public init(auth: FiveHundredspxWebAuthentication, authURL: URL?, redirectURL: URL?, completionHandler: AuthViewController.CompletionHandler? = nil) {
        self.auth = auth
    ...
    override public func webView(_: WKWebView, decidePolicyFor navigationAction: WKNavigationAction, decisionHandler: @escaping (WKNavigationActionPolicy) -> Void) {
        ...
        if let urlString = navigationAction.request.url?.absoluteString, let currentRedirectURL = redirectURL?.absoluteString, !urlString.isEmpty {
            if urlString == currentRedirectURL {
                showHUD()
                loggedIn = true
